...
function display_all_cart_items() {
  ...
  if (cart_items.length === 0) {
    ...
    cart_items.forEach((ele) => {
      ...
      var item_image_url = ele.image1;

      var item_price = ele.price1;
      ...
      var item_total_price_in_string =
        "Rs. " + add_commas_to_price_string(item_total_price + "");
      ...
      new_div.innerHTML = `<div class="first_sixty_percent">
        ...
        <div class="product_description">
            <span class="mobile_label">Product</span>
    ...
    <div class="quantity_div">
        <span class="mobile_label">Quantity</span>
        <div class="product_quantity_box">
            <div class="product_quantity_box_minus_box"><svg class="product_quantity_box_minus_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M432 256c0 17.7-14.3 32-32 32L48 288c-17.7 0-32-14.3-32-32s14.3-32 32-32l352 0c17.7 0 32 14.3 32 32z"/></svg></div>
            <div class="product_quantity_box_number"><span id="${item_quantity_span_id}">${item_quantity_string}</span></div>
            <div class="product_quantity_box_plus_box"> <svg class="product_quantity_box_plus_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg> </div>
        </div>
    ...
    <div class="price_div">
        ...
        <div class="remove_button_container">        
            <a id="${item_remove_tag_id}" href="#" class="remove_button">Remove</a>
        </div>
        </div>`;

      document.querySelector("#items").append(new_div);

      // attach event listeners instead of inline onclicks
      const minusBtn = new_div.querySelector(".product_quantity_box_minus_box");
      if (minusBtn)
        minusBtn.addEventListener("click", () =>
          decrease_item_quantity(item_id)
        );
      const plusBtn = new_div.querySelector(".product_quantity_box_plus_box");
      if (plusBtn)
        plusBtn.addEventListener("click", () =>
          increase_item_quantity(item_id)
        );
      const removeBtn = new_div.querySelector(`#${item_remove_tag_id}`);
      if (removeBtn)
      ...
      document.getElementById("cart_total").innerText =
        add_commas_to_price_string(total_cart_price + "");
    });
  }
}
...
function increase_item_quantity(product_id) {
  ...
  var new_total_price =
    parseInt(
      remove_commas_from_price_string(
        document.getElementById("span_unit_price_" + product_id).innerText
      ) + ""
    ) * new_quantity;

  var new_total_price_string =
    "Rs. " + add_commas_to_price_string(new_total_price);

  document.getElementById(
    "span_total_price_of_product_" + product_id
  ).innerText = new_total_price_string;

  document.getElementById(span_id).innerText = new_quantity_string;

  var current_cart_price = parseInt(
    remove_commas_from_price_string(
      document.getElementById("cart_total").innerText
    )
  );
  ...
  document.getElementById("cart_total").innerText =
    "" + add_commas_to_price_string(new_cart_price);
